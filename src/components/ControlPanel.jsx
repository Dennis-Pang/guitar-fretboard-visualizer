  ...
  return (
    <div className="control-panel w-full max-w-[1200px] mx-auto mb-8 animate-fade-in-up px-4 md:px-0">
      ...
      <div className="flex flex-col items-center justify-center mb-8">
        <h1 className="text-3xl md:text-5xl font-black text-text-main mb-3 tracking-tight text-center uppercase font-heading">
          Guitar Scale Visualizer
        </h1>
        <div className="h-1 w-24 bg-accent rounded-full mb-4"></div>
        <p className="text-text-muted text-sm font-bold tracking-[0.2em] uppercase opacity-80 text-center">
          {rootNote} {currentMode?.name || ''}
        </p>
      </div>

      {/* 主控制栏 - 简约暗黑风格 */}
      <div className="bg-white/95 border border-slate-200 rounded-3xl p-6 md:p-8 shadow-xl shadow-indigo-100">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 lg:gap-6">
          ...
          <div className="flex flex-col gap-2 lg:min-w-[80px]">
            <label className="text-[10px] font-bold text-accent uppercase tracking-widest pl-1">Root</label>
            <div className="relative group">
              <select
                value={rootNote}
                onChange={(e) => onRootNoteChange(e.target.value)}
                className="w-full appearance-none bg-white text-text-main text-sm font-semibold border border-slate-200 rounded-2xl px-4 py-3 shadow-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all cursor-pointer hover:border-slate-300"
              >
                {ROOT_NOTES.map(note => (
          ...
          <div className="flex flex-col gap-2 flex-1 lg:min-w-[180px]">
            <label className="text-[10px] font-bold text-accent uppercase tracking-widest pl-1">System</label>
            <div className="relative">
              <select
                value={scaleSystem}
                onChange={(e) => onScaleSystemChange(e.target.value)}
                className="w-full appearance-none bg-white text-text-main text-sm font-semibold border border-slate-200 rounded-2xl px-4 py-3 shadow-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all cursor-pointer hover:border-slate-300 truncate"
              >
          ...
          <div className="flex flex-col gap-2 flex-[1.5] lg:min-w-[200px]">
            <label className="text-[10px] font-bold text-accent uppercase tracking-widest pl-1">Mode</label>
            <div className="relative">
              <select
                value={mode}
          ...
          <div className="flex flex-col gap-2 lg:min-w-[140px]">
            <label className="text-[10px] font-bold text-accent uppercase tracking-widest pl-1">Display</label>
            <div className="flex bg-slate-100 rounded-2xl p-1 border border-slate-200 h-[46px]">
              <button
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {chordSuggestions.map((chord, index) => (
                  <div
                    key={index}
                    className="px-4 py-1.5 bg-slate-50 border border-slate-200 rounded-full text-xs font-mono text-accent shadow-sm"
                  >
